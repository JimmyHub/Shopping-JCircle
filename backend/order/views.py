...
class OrdersView(GenericAPIView):
    ...
    def get(self, request, keyword=None, mode=None):
        ...
        if keyword == '0':
            # 賣場總覽獲取
            if mode == '1':
                list_g = OrdersFiles.objects.filter(sales_id=request.user.username)
            # 個人總覽獲取
            elif mode == '0':
                list_g = OrdersFiles.objects.filter(buyer_id=request.user.username)
            if list_g:
                list_orders = []
                for i in list_g:
    ...
    @login_check('PUT')
    def put(self, request, keyword=None, mode=None):
        json_str = request.body
        if not json_str:
    ...
    def delete(self, request, keyword=None, mode=None):
        ...
        if not orders:
            result = {'code': 410, 'error': 'Something must be wrong'}
            return JsonResponse(result)
        if request.user.username != orders[0].buyer.username:
